    ...
    async confirmUpload(row, index) {
      ...
      if (file) {
        ...
        try {
          ...
          const storagePath = `${this.project}/${this.project}_data/Budget_tab/${row.id}.pdf`;
          ...
          await addDoc(collection(db, collectionPath), {
            ...
            PDF: downloadURL,
            remarks: "Routed to Regional Director",
            timestamp: timestamp,
          });

          // Update 'purchase_requests' collection to mark the request as completed
          const purchaseRequestPath = `${this.project}/${this.project}_data/TOD_tab/${row.id}`;
